    ...
    for eachLine in data:
        if len(eachLine.split(',')) != 17: continue
            ...
            if counter > length and  counter >= 3050:
                ...
                y0 = np.append(y0,np.array([int(ch1)]))
                y1 = np.append(y1,np.array([int(ch2)]))
    diff_y0 = np.diff(y0) + 5
    diff_y1 = np.diff(y1) + 1
    mx = max(x)
    ax1.clear()
    ax1.plot(x,diff_y0, label='up/down')
    ax1.plot(x,diff_y1, label='kicker')
    ax1.set_xlim([mx-300,mx])
    ax1.set_ylim([0,10])
    #ax1.set_ylim(bottom = 0)
    plt.grid()
    plt.title("Beam Control Information #{0}".format(args[1]))
